...
# KO Drum Configuration File (Optimized based on sensitivity analysis)
...
OPERATING_CONDITIONS = {
    ...
    'temperature': 20,  # °C
    'temperature_K': 293.15,  # K (50°C + 273.15)
}

# Vessel Dimensions (optimized from sensitivity analysis)
VESSEL_DIMENSIONS = {
    'diameter': 1.22,  # meters (optimized)
    'height': 6.12,  # meters (optimized)
    'liquid_holdup_height': 0.43,  # meters (scaled proportionally)
    'total_volume': np.pi * 1.22**2 * 6.12 / 4,  # m³ (π * d² * h / 4)
}
...
# Design Parameters (adjusted for optimized configuration)
DESIGN_PARAMETERS = {
    ...
    'min_liquid_level': 0.17,  # m (scaled for new diameter)
    'max_liquid_level': 0.67,  # m (scaled for new diameter)
...
# Control Parameters (adjusted setpoints for optimized configuration)
CONTROL_PARAMETERS = {
    'level_control_type': 'PID',
    'level_control_setpoint': 0.42,  # m (scaled for new diameter)
...
if __name__ == "__main__":
    print("Optimized KO Drum Configuration Summary:")
    ...
    print(f"Diameter: {VESSEL_DIMENSIONS['diameter']:.2f} m")
    print(f"Height: {VESSEL_DIMENSIONS['height']:.2f} m")
    print(f"L/D Ratio: {VESSEL_DIMENSIONS['height']/VESSEL_DIMENSIONS['diameter']:.2f}")
    print(f"Total Volume: {VESSEL_DIMENSIONS['total_volume']:.2f} m³")
